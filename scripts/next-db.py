...
if not (data/'PG_VERSION').exists():
    ...
    with tempfile.NamedTemporaryFile(mode='w',prefix='wayline_next-pg-') as f:
        f.write(password+'\n');f.flush()
        run('initdb','-D',data,'-U','wayline_next','-A','scram-sha-256','--pwfile='+f.name,'--encoding=UTF8','--locale=C.UTF-8')
if not running():
    # TCP loopback only; avoid long project paths exceeding Unix socket path limits.
    options=shlex.join(['-p',str(port),'-h','127.0.0.1','-k',''])
    run('pg_ctl','-D',data,'-l',state/'postgres-next.log','-o',options,'-w','start')
env={**os.environ,'PGPASSWORD':password}
connection=['-h','127.0.0.1','-p',str(port),'-U','wayline_next']
exists=subprocess.check_output([str(bin/'psql'),*connection,'-d','postgres','-tAc',"SELECT 1 FROM pg_database WHERE datname='wayline_next'"],env=env,text=True).strip()
if exists!='1':run('createdb',*connection,'wayline_next',env=env)
run('psql',*connection,'-d','wayline_next','-v','ON_ERROR_STOP=1','-c','CREATE EXTENSION IF NOT EXISTS btree_gist;',env=env)
test_exists=subprocess.check_output([str(bin/'psql'),*connection,'-d','postgres','-tAc',"SELECT 1 FROM pg_database WHERE datname='wayline_next_test'"],env=env,text=True).strip()
if test_exists!='1':run('createdb',*connection,'wayline_next_test',env=env)
print('Test database: wayline_next_test (same connection credentials).')
print('Ready: 127.0.0.1:55433 / wayline_next, user wayline_next. Local password: NextDb123!')
